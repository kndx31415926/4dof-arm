...
%% -------- 控制器（严格推理） --------
areg = struct('BETA',0.85,'A_DOT_UP',2.5,'A_DOT_DN',5.0);
if isfield(cfg,'ALPHA_REG')
    fns = fieldnames(areg);
    for i=1:numel(fns), if isfield(cfg.ALPHA_REG,fns{i}), areg.(fns{i}) = cfg.ALPHA_REG.(fns{i}); end, end
end
opts = struct();
opts.Ts      = cfg.TS;
opts.fric    = cfg.FRIC;
...
bus = get_bus_from_cfg(cfg, limits.P_total_max);  % BUS 结构（总线效率/制动电阻等）
...
% 将 payload 注入末端刚体，并返回 1×10 向量 [m, com(1:3), Ixx Iyy Izz Ixy Ixz Iyz]
% 若未提供 pay（或字段缺失），对应项置 0，不报错。
    robot_out = robot_in;
    pay_vec   = zeros(1,10);

    if ~isempty(pay) && isstruct(pay)
        % 读入并容错
        if isfield(pay,'mass')    && ~isempty(pay.mass),    m   = double(pay.mass);    else, m   = 0; end
        if isfield(pay,'com')     && ~isempty(pay.com),     com = double(pay.com(:));  else, com = [0;0;0]; end
        if isfield(pay,'inertia') && ~isempty(pay.inertia), I6  = double(pay.inertia(:));
        else, I6 = zeros(6,1); end

        % 截断/补齐
        com = [com; 0; 0; 0]; com = com(1:3);
        I6  = [I6;  zeros(6,1)]; I6 = I6(1:6);

        % 返回 1×10
        pay_vec = [m, reshape(com,1,[]), reshape(I6,1,[])];

        % 尝试把工具惯量写进 rigidBodyTree（无则跳过）
        try
            b = robot_out.Bodies{end};
            b.Mass          = m;
            b.CenterOfMass  = reshape(com,1,[]);
            b.Inertia       = reshape(I6,1,[]);
        catch
            % 某些版本/对象无 Bodies 字段，静默跳过
        end
    end
end
