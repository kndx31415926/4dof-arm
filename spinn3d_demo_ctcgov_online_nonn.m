...
bus = get_bus_from_cfg(cfg, limits.P_total_max);
...
% ---------- BUS 口径：功率记账 ---------- % BUS:
function [Ppos, Pneg, Pgrid, Pdump, Paxis_pos] = spinn3d_bus_power(tau, vel, bus)
    pw = tau(:)'.*vel(:)';              % 逐轴功率
    Paxis_pos = max(pw,0);
    Ppos = sum(Paxis_pos);
    Pneg = sum(max(-pw,0));             % 再生功（取正值表示幅度）
    share = min(bus.eta_share * Pneg, Ppos);
    Pgrid = max(Ppos - bus.eta_share*Pneg, 0);
    Pdump = max(Pneg - share, 0);
    if isfield(bus,'P_brk_peak') && isfinite(bus.P_brk_peak)
        Pdump = min(Pdump, bus.P_brk_peak);
    end
end
...
% 注入 payload（若提供），返回 1×10：[m, com(1:3), Ixx Iyy Izz Ixy Ixz Iyz]
    robot_out = robot_in;
    pay_vec   = zeros(1,10);
    if isempty(pay), return; end

    if isnumeric(pay)
        v = double(pay(:)).'; if numel(v) < 10, v = [v, zeros(1,10-numel(v))]; end
    ...
    elseif isstruct(pay)
        ...
        about = getfield_def(pay,'about','com');
        mode  = lower(getfield_def(pay,'mode','add'));
    else
        return;
    end
    ...
    try
        ...
        switch mode
            case 'replace'
                m_new  = m;  c_new = com(:).';  I0_new = I1;
            otherwise
                m_new  = m0 + m;
                if m_new <= 0, c_new = [0 0 0]; else, c_new = (m0*c0 + m*com(:).') / m_new; end
                I0_new = I0 + I1;
        end

        b.Mass = m_new;
        b.CenterOfMass = c_new;
        b.Inertia      = ImatTo6(I0_new).';
    catch
        % 忽略注入失败（仅影响动力学），与 NN 版口径对齐
    end
end
function I = I6ToMat(v6), I = [ v6(1) -v6(4) -v6(5); -v6(4)  v6(2) -v6(6); -v6(5) -v6(6)  v6(3)]; end
function v6 = ImatTo6(I), v6 = [I(1,1), I(2,2), I(3,3), -I(1,2), -I(1,3), -I(2,3)]; end
