...
%   BASE_NAME     : 基座名（可空；兼容老接口）
    ...
    for k=1:Ns-1
        ...
        Jlin = J(1:3,:);                   % 位置在前（兼容 getTransform_compat 的行序）
        Jang = J(4:6,:);                   % 部分版本是 [ang; lin]，若与你环境相反请对调两行
        if size(Jlin,1)~=3 || size(Jang,1)~=3
            % 兼容某些版本：J(4:6,:) 是线速度
            Jlin = J(4:6,:);
            Jang = J(1:3,:);
        end
...
function [Jlin, Jang] = splitJ(J)
    % 尝试自动识别行序，保证 Jlin/Jang 均为 3×n
    top = J(1:3,:); bot = J(4:6,:);
    if size(top,1)~=3 || size(bot,1)~=3
        Jlin = J(4:6,:); Jang = J(1:3,:); return;
    end
    % 经验判定：线速度雅可比范数通常更大
    if norm(bot,'fro') > norm(top,'fro')
        Jlin = bot; Jang = top;
    else
        Jlin = top; Jang = bot;
    end
